...
4. 模型功能
- 左侧列出 YAML **配置位**（如 `chat-model`），与「模型映射」一一对应
- 选中配置位后，右侧上方只读展示 **当前 model_name** 与来源路径
- 在右侧编辑该配置位在项目中的实际用途说明（支持 `# 1.` 列表写法）
- 点击「保存当前」或「保存全部」写入 `../.local/model-usage.json`（本机，不提交 Git；键为配置位名）
- 样例可参考 `../model-usage.example.json`；切换配置位前若有未保存修改会提示
- 修改 YAML 后点「刷新模型列表」同步；已删除配置位若仍有笔记会在刷新时被清理
- 若本地仍是旧版 v1（键为 model_name），首次刷新会自动迁移到 v2（键为配置位名）；同一 model_name 绑定多个配置位时说明会复制到各配置位，不在 YAML 中的孤立 model 说明会被丢弃并弹窗提示
