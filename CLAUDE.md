...
- **`.claude/settings.json`** — project-level pre-authorized CLI permissions (build/test/jar extraction for LangChain4j class patching) and additional working directories (`D:\pptx-work`, etc.). Local overrides in `.claude/settings.local.json` take precedence.
...
> Note: `.mdc` files are Cursor-specific format; Claude Code relies on this CLAUDE.md transcription only.
