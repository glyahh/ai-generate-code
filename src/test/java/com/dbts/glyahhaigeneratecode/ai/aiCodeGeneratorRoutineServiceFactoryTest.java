...
@Slf4j
@SpringBootTest
class aiCodeGeneratorRoutineServiceFactoryTest {
    @Resource
    ...
    public void testConcurrentRoutingCalls() throws InterruptedException {
        ...
        for (int i = 0; i < prompts.length; i++) {
            ...
            threads[i] = Thread.ofVirtual().start(() -> {
                aiCodeGeneratorRoutineService service = aiCodeGeneratorRoutineServiceFactory.createAiCodeGeneratorRoutineService();
                var result = service.aiCodeGeneratorRoutine(prompt);
